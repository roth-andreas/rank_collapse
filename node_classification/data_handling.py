...
def get_data(name, split=0):
    ...
    if name in ['chameleon','squirrel']:
        ...
        dataset = PPI(root=path, split='train')
    else:
    ...
    data = dataset[0]
    print(data)
    if not name in ['Cora', 'Citeseer', 'Pubmed','ppi']:
        ...
        if name in ['chameleon', 'squirrel','cornell', 'texas', 'wisconsin','film','snap-patents','arxiv-year']:
...
def load_arxiv_year_dataset(nclass=5):
    ...
    dataset.label = torch.as_tensor(label).reshape(-1, 1).long()
    return dataset
...
def load_snap_patents_mat(nclass=5):
    if not path.exists(f'./dataset/snap_patents.mat'):
        p = dataset_drive_url['snap-patents']
        print(f"Snap patents url: {p}")
        gdown.download(id=dataset_drive_url['snap-patents'], \
                       output=f'./dataset/snap_patents.mat', quiet=False)

    fulldata = scipy.io.loadmat(f'./dataset/snap_patents.mat')
